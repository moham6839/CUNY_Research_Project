---
title: "2022-23 Data Extraction and Modeling"
...
nba_game_log <- nba_leaguegamelog(season = 2023) # 2023-24 season
...
write_csv(game_stint_rapm_data,
          ...
          "data/nba_2324_season_rapm_data.csv.gz")
...
```{r}
model_2324 <- lm(margin ~ . - 1, data = df_2324[, 7:579])
```


```{r}
predicted_values <- fitted.values(model_2324)
```
...
```{r}
sqrt(mean(model_2324$residuals^2))
```
