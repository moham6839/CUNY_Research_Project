---
title: "2021-22 Data Extraction and Modeling"
...
nba_game_log <- nba_leaguegamelog(season = 2021) # 2021-22 season
...
write_csv(season_stint_data,
          "data/nba_2122_season_stints.csv")
...
write_csv(game_stint_rapm_data,
          ...
          "data/nba_2122_season_rapm_data.csv.gz")


# Get the player ids to join for viewing later ----------------------------

nba_players <- nba_commonallplayers(season = 2021)
...
write_csv(player_table,
          "data/nba_2122_player_table.csv")
```



```{r}
df_2122 <- game_stint_rapm_data
```
...
```{r}
season_stint_2122 <- read_csv("data/nba_2122_season_stints.csv")
season_stint_2122
```


```{r}
player_table_2122 <- read_csv("data/nba_2122_player_table.csv")
player_table_2122
```
...
```{r}
model_2122 <- lm(margin ~ . - 1, data = df_2122[, 7:612])
```


```{r}
predicted_values <- fitted.values(model_2122)
```
