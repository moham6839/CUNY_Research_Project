...
nba_game_log <- nba_leaguegamelog(season = 2022) # 2022-23 season
...
```{r}
df <- read_csv("data/nba_2223_player_table.csv")
```


```{r}
# Look at huber regression - lasso for large values, elastic for small values
# Use package for ridge, lasso, elasticnet
# Cross-Validation
# Compute minutes for each player and filter out players - **after creating models**
```

```{r}
df_2223 <- game_stint_rapm_data
```
