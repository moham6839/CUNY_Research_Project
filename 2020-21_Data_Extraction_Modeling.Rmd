---
title: "2020-21 Data Extraction and Modeling"
...
nba_game_log <- nba_leaguegamelog(season = 2020) # 2020-21 season
...
write_csv(season_stint_data,
          "data/nba_2021_season_stints.csv")
...
write_csv(game_stint_rapm_data,
          ...
          "data/nba_2021_season_rapm_data.csv.gz")


# Get the player ids to join for viewing later ----------------------------

nba_players <- nba_commonallplayers(season = 2020)
...
write_csv(player_table,
          "data/nba_2021_player_table.csv")
```

```{r}
df_2020 <- game_stint_rapm_data
```
...
```{r}
season_stint_2021 <- read_csv("data/nba_2021_season_stints.csv")
season_stint_2021
```
...
```{r}
player_table_2021 <- read_csv("data/nba_2021_player_table.csv")
player_table_2021
```
...
```{r}
model_2021 <- lm(margin ~ . - 1, data = df_2021[, 7:547])
```


```{r}
predicted_values <- fitted.values(model_2021)
```
